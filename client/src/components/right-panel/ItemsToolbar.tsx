...
// PURPOSE: Minimalist icon bar with inline expanding panels for Items tab
...
export function ItemsToolbar(props: ItemsToolbarProps) {
  ...
  /** WHY: Click-outside closes any open panel */
  useEffect(() => {
  ...
  return (
    <div ref={barRef} className="sticky top-0 z-10 bg-[var(--color-bg-card)]">
      ...
      <div className="flex items-center gap-2 px-[var(--spacing-3xl)] py-[var(--spacing-base)] border-b border-[var(--color-gold-subtle)]">
        ...
        {isFiltered && (
          <span className="text-[11px] text-[var(--color-text-muted)]">{filteredCount} of {totalCount}</span>
        )}
      </div>

      {/* WHY: Inline panel (not absolute) — avoids overflow clipping from parent scroll container.
       * AnimatePresence with height animation smoothly pushes table content down. */}
      <AnimatePresence initial={false}>
        {openPanel && (
          <motion.div
            ...
            className="overflow-hidden border-b border-[var(--color-gold-subtle)]"
          >
            <div className="px-[var(--spacing-3xl)] py-3 max-h-[240px] overflow-y-auto">
  ...
  return (
    ...
    >
      <button type="button" onClick={handleExpand}
        className="absolute left-0 w-7 h-7 flex items-center justify-center shrink-0 text-[var(--color-text-muted)]" aria-label="Search items">
        <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
function ToolbarIcon({ panel, openPanel, onToggle, badge, label, icon }: {
  ...
  badge: number | null; label?: string; icon: React.ReactNode;
}) {
  ...
  return (
    <button type="button" onClick={() => onToggle(panel)}
      ...
      aria-label={panel} aria-expanded={isOpen}>
      <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        {icon}
      </svg>
      {label && !isOpen && (
